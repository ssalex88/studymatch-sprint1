...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const usuarioParseado = datosGuardados ? JSON.parse(datosGuardados) : null;
    ...
    return (
      <div className="min-h-full w-full flex items-center justify-center bg-slate-50 px-4 py-16">
        <div className="w-full max-w-md text-center bg-white border border-slate-100 shadow-xl rounded-2xl px-8 py-12">
          ...
          <p className="font-sans text-sm font-semibold text-red-600 mb-3">Error 403</p>
          <p className="font-sans text-sm text-slate-500">
            No cuentas con los permisos necesarios para visualizar este panel.
            Esta sección está reservada exclusivamente para usuarios con rol
            de Administrador.
          </p>
        </div>
      </div>
                    ...
                    usuarios.map((u) => (
                      <tr key={u.idUsuario} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full ${obtenerClasesRol(
                              u.rol
                            )}`}
                          >
