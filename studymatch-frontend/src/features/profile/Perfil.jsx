...
export default function Perfil() {
  ...
  // Carga los datos del usuario logueado desde localStorage al montar el componente.
  useEffect(() => {
    ...
    if (datosGuardados) {
      ...
      setFormData({
        carrera: usuarioParseado.carrera || "",
        ciclo: usuarioParseado.ciclo || "",
        codigoAlumno: usuarioParseado.codigoAlumno || "",
        biografia: usuarioParseado.biografia || "",
      });
    }
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    setGuardando(true);
    try {
      const response = await fetch(`${ENV.API_URL}/usuarios/${usuario.idUsuario}`, {
        method: "PUT",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.mensaje || "No se pudo actualizar el perfil.");
      }
      ...
      setError(err.message || "Ocurrió un error inesperado al guardar los cambios.");
    } finally {
      setGuardando(false);
    }
  };
    ...
    <div className="min-h-full w-full bg-slate-50 px-4 py-10 flex items-start justify-center">
      <div className="w-full max-w-lg">
        <div className="text-center mb-6">
          ...
          <h1 className="font-title text-2xl font-bold text-slate-800">Mi perfil académico</h1>
          <p className="font-sans text-sm text-slate-500 mt-1">
            Actualiza tu información para que otros estudiantes te conozcan mejor
          </p>
        </div>
        ...
        <div className="bg-white shadow-xl rounded-2xl px-6 py-8 sm:px-8 border border-slate-100">
          ...
          <div className="flex items-center gap-4 mb-6 pb-6 border-b border-slate-100">
            <div className="w-14 h-14 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-700 font-title font-bold text-lg">
              {usuario.nombreCompleto ? usuario.nombreCompleto.charAt(0).toUpperCase() : "?"}
            </div>
            <div>
              <p className="font-title font-semibold text-slate-800">{usuario.nombreCompleto}</p>
              <p className="font-sans text-sm text-slate-500">{usuario.correoInstitucional}</p>
